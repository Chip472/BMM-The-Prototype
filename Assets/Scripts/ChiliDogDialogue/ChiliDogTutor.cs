...
public class ChiliDogTutor : MonoBehaviour
{
    ...
    IEnumerator TurnOffTutor()
    {
        yield return new WaitForSeconds(5f);

        // Only hide if the tutorial is still on the same index or hasn't progressed
        if (isTutorial && index < lines.Length)
        {
            textboxTutor.SetActive(false);
        }
    }
